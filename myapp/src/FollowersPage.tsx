...
export const FollowersPage: React.FC = () => {
  ...
  const fetchData = useCallback(async (currentUser: FirebaseUser | null) => {
    ...
    try {
      const response = await fetch(`${BACKEND_API_URL}/api/users/${userId}/followers`, { headers });
      if (!response.ok) throw new Error('フォロワーの取得に失敗しました。');
  ...
  return (
    <div>
       <div style={{ padding: '10px', display: 'flex', alignItems: 'center', borderBottom: '1px solid #38444d' }}>
        ...
        <h2 style={{ margin: 0, padding: 0, border: 'none' }}>フォロワー</h2>
      </div>
